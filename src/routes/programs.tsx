  ...
  return (
    <PageShell eyebrow="WHAT WE DO" title="Our Programs" intro="Five integrated programs working together to uplift the most marginalized across Pakistan.">
      <div className="grid md:grid-cols-2 gap-8">
        {programs.map(({ img, icon: Icon, title, body }) => (
          <article key={title} className="bg-card rounded-xl overflow-hidden shadow-card hover:shadow-elegant transition-smooth group">
            ...
            <div className="p-7">
              <div className="flex items-center gap-3">
                <span className="h-10 w-10 rounded-full bg-accent-soft text-accent grid place-items-center"><Icon className="h-5 w-5" /></span>
                <h3 className="text-xl font-bold text-foreground">{title}</h3>
              </div>
              <p className="mt-4 text-muted-foreground leading-relaxed">{body}</p>
              <Link to="/get-involved" className="mt-5 inline-flex items-center gap-1 text-sm font-bold text-primary hover:text-accent">
